...
test('basic API', async t => {
    ...
    class MyExtension {}
    setScript('https://sailfish-studio.org/1.js', () => {
    ...
    const extensions = await UnsandboxedExtensionRunner.load('https://sailfish-studio.org/1.js', vm);
...
test('multiple VMs loading extensions', async t => {
    ...
    let api1 = null;
    setScript('https://sailfish-studio.org/1.js', async () => {
    ...
    let api2 = null;
    setScript('https://sailfish-studio.org/2.js', () => {
    ...
    const extensions = await Promise.all([
        UnsandboxedExtensionRunner.load('https://sailfish-studio.org/1.js', vm1),
        UnsandboxedExtensionRunner.load('https://sailfish-studio.org/2.js', vm2)
    ]);
...
test('register multiple extensions in one script', async t => {
    ...
    class Extension1 {}
    class Extension2 {}
    setScript('https://sailfish-studio.org/multiple.js', () => {
        global.Scratch.extensions.register(new Extension1());
        global.Scratch.extensions.register(new Extension2());
    });
    const extensions = await UnsandboxedExtensionRunner.load('https://sailfish-studio.org/multiple.js', vm);
    t.equal(extensions.length, 2);
...
test('extension error results in rejection', async t => {
    const vm = new VirtualMachine();
    try {
        await UnsandboxedExtensionRunner.load('https://sailfish-studio.org/404.js', vm);
...
test('ScratchX', async t => {
    const vm = new VirtualMachine();
    setScript('https://sailfish-studio.org/scratchx.js', () => {
        const ext = {
            test: () => 2
        };
        const descriptor = {
    ...
    const extensions = await UnsandboxedExtensionRunner.load('https://sailfish-studio.org/scratchx.js', vm);
    t.equal(extensions.length, 1);
...
test('rewriteExtensionURL', async t => {
    ...
    setScript('https://sailfish-studio.org/rewritten.js', () => {
    ...
    setScript('https://sailfish-studio.org/original.js', () => {
    ...
    vm.securityManager.rewriteExtensionURL = url => {
        if (url === 'https://sailfish-studio.org/original.js') {
            return 'https://sailfish-studio.org/rewritten.js';
        }
        return url;
    };

    await vm.extensionManager.loadExtensionURL('https://sailfish-studio.org/original.js');

    t.ok(createdRewrittenExtension, 'used rewritten extension');
    t.ok(vm.extensionManager.isExtensionURLLoaded('https://sailfish-studio.org/original.js'), 'marks original URL as loaded');
    t.notOk(vm.extensionManager.isExtensionURLLoaded('https://sailfish-studio.org/rewritten.js'), 'does not mark new URL as loaded');
    t.end();
});
