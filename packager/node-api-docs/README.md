...
```bash
npm install --save-exact @sailfish-studio/packager
```
...
Large assets are cached in `node_modules/@sailfish-studio/packager/.packager-cache`. You may want to periodically clean this folder.
...
```js
const Packager = require('@sailfish-studio/packager');
```
