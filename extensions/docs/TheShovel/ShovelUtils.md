...
```scratch
import image from [https://extensions.sailfish-studio.org/dango.png] name [Dango]
```
...
```scratch
import sound from [https://extensions.sailfish-studio.org/meow.mp3] name [Meow]
```
...
```scratch
import project from [https://extensions.sailfish-studio.org/samples/Box2D.sb3]
```
...
```scratch
load extension from [https://extensions.sailfish-studio.org/utilities.js]
```

Imports any extension from a link. Extensions from the [Extension Gallery](https://extensions.sailfish-studio.org) can run unsandboxed, and don't require permission to import.
...
Gets the brightness of a hex value. Reports a decimal between 0 and 255. To transfer this to a value between 0 and 100 (what Sailfish-Studio uses), divide the output of the block by 2.55 and round.
