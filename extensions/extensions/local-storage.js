...
(function (Scratch) {
  ...
  const STORAGE_PREFIX = "extensions.sailfish-studio.org/local-storage:";
    ...
    try {
      if (Object.keys(namespaceValues).length > 0) {
        localStorage.setItem(
          getStorageKey(),
          JSON.stringify({
            // If we find that sailfish-studio.org is commonly running out of shared space in local storage,
  ...
  class LocalStorage {
    getInfo() {
      return {
        ...
        docsURI: "https://extensions.sailfish-studio.org/local-storage",
        blocks: [
          {
