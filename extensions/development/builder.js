...
import ExtendedJSON from "@sailfish-studio/json";
...
class HomepageFile extends BuildFile {
  constructor(
    ...
    this.host =
      mode === "development"
        ? "http://localhost:8000/"
        : "https://extensions.sailfish-studio.org/";
  }
  ...
  getRunExtensionURL(extensionSlug) {
    return `https://sailfish-studio.org/editor?extension=${this.getFullExtensionURL(
      extensionSlug
    )}`;
  }
  ...
  getRunSampleURL(sampleFile) {
    ...
    return `https://sailfish-studio.org/editor?project_url=${this.host}${path}`;
  }
...
class SitemapFile extends BuildFile {
  ...
  read() {
    ...
    xml += Object.keys(this.build.files)
      ...
      .map((path) => `https://extensions.sailfish-studio.org${path}`)
...
class SampleFile extends BuildFile {
  ...
  validate() {
    ...
    for (const url of urls) {
      if (
        !url.startsWith("https://extensions.sailfish-studio.org/") ||
        !url.endsWith(".js")
      ) {
...
class Builder {
  ...
  async build() {
    ...
    const scratchblocksPath = urlUtil.fileURLToPath(
      import.meta.resolve("@sailfish-studio/scratchblocks")
    );
