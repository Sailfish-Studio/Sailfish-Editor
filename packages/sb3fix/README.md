...
https://sailfish-studio.github.io/sb3fix/
...
```bash
npm install @sailfish-studio/sb3fix
```

Then use from Node.js:

```js
const sb3fix = require('@sailfish-studio/sb3fix');
const fs = require('fs');
...
const run = async () => {
  ...
  const options = {
    ...
    // Values: 'scratch' (default), 'sailfish-studio'
    platform: 'scratch'
  };
...
```bash
git clone https://github.com/Sailfish-Studio/sb3fix.git
cd sb3fix
npm ci
```
