<script lang="ts">
  ...
  const loadProject = async () => {
    ...
    // @ts-expect-error
    await import('@sailfish-studio/packager/dist/scaffolding/scaffolding-full');
    // @ts-expect-error
    ...
    scaffolding.setExtensionSecurityManager({
      getSandboxMode: (extension: string) => {
        if (extension.startsWith('https://extensions.sailfish-studio.org/')) return 'unsandboxed';
        return 'iframe';
      },
      canLoadExtensionFromProject: (extension: string) => {
        ...
        if (extension.startsWith('https://extensions.sailfish-studio.org/')) {
          return true;
        }
        throw new Error(`Loading extensions from places other than extensions.sailfish-studio.org not supported yet: ${extension}`);
      }
    });
